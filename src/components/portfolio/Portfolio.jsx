...
const PROJECTS = [
  {
    id: 1,
    title: "Agency site with AI intake",
    copy:
...
export default function Portfolio() {
  ...
  const gctx = gsap.context(() => {
    // ---> JS puro: niente generics
    const cardsEls = gsap.utils.toArray(".pf-card"); // Array<Element>
    const cards = cardsEls.filter(el => el instanceof HTMLElement); // Array<HTMLElement>
        ...
        {PROJECTS.map((p) => (
          <article className="pf-card" key={p.id} style={{ "--accent": p.color }}>
            ...
            <div className="pf-actions">
              {p.repo && (
                <a className="pf-btn" href={p.repo} target="_blank" rel="noreferrer">Repo</a>
              )}
              {p.demo && (
                <a className="pf-btn pf-btn--ghost" href={p.demo} target="_blank" rel="noreferrer">Demo</a>
              )}
            </div>
